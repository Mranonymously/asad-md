...
if (!fs.existsSync(CREDS)) {
  ...
  console.log("♻️ ASAD-MD session restored successfully");
}
...
async function getProfilePicture(sock, jid) {
  try {
    ...
    return 'https://h.uguu.se/KsZZhDfs.jpeg; // Default image
  }
}
...
async function connectToWA() {
  ...
  console.log(`🤖 ASAD-MD Connecting with prefix: "${prefix}"`);
  ...
  sock.ev.on('connection.update', (update) => {
    ...
    if (connection === 'close') {
      ...
      console.log("✅ ASAD-MD Bot connected to WhatsApp!");
      ...
      const imageUrl = 'https://h.uguu.se/KsZZhDfs.jpeg';
      
      try {
        // Send to owner with image
        sock.sendMessage(ownerNumber[0] + '923359216317@s.whatsapp.net', {
          image: { url: imageUrl },
          caption: aliveMsg
        }).catch(() => {
